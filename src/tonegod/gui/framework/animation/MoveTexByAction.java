...
public class MoveTexByAction extends TemporalAction {
	...
	protected void begin() {
		...
		nextPercent = 0;
		if (autoReverse) {
			if (initX == -1) initX = ((QuadData)quad).tcOffsetX;
			if (initY == -1) initY = ((QuadData)quad).tcOffsetY;
			if (initDuration == -1)	{
	...
	protected void update(float percent) {
		...
		((QuadData)quad).tcOffsetX += (amountX * nextPercent);
		((QuadData)quad).tcOffsetY += (amountY * nextPercent);
	}
	...
	protected void end() {
		if (autoReverse && cycles == 0) {
			...
			((QuadData)quad).tcOffsetX = initX;
			((QuadData)quad).tcOffsetY = initY;
			amountX = -amountX;
			amountY = -amountY;
